...
do
  ...
  do
     ...
     echo sbatch --job-name="${qrydtset}_$i$reg" --time=1:00:00 --mem=32000 --cpus-per-task=4 --output=$output plt_query.sh $i SGD "${pltResDir}/$qrydtset/$folder" "$dataset/wilcox_res" $ldareg $epochs ${gene_sampling} ${cell_sampling} ${web_layer_sizes} 
  done
  for i in 600 1000
  do
     echo sbatch --job-name="${qrydtset}_$i" plt_query $i
     output=${pltResDir}/$qrydtset/$folder/query_logs/slurm_${i}-%A_%a.ou
     echo sbatch --job-name="${qrydtset}_$i$reg" --mem=32000 --cpus-per-task=4 --array=0-8:2 --time=1:00:00 --output=$output plt_query.sh $i SGD ${pltResDir}/$qrydtset/$folder "$dataset/wilcox_res" $ldareg $epochs ${gene_sampling} ${cell_sampling} ${web_layer_sizes}
  done
  #break
done
