...
def run(args):
    ...
    if restart==0:
        if args.gene_info:
            gene_info_df=pd.read_pickle(args.gene_info)
        ...
        if args.gene_info:
            ...
            gene_info=torch.from_numpy(np_gen_info_vals)
    ...
    if restart==0:
        ...
        swarm_ldas.fit(X_fit.to(device),y_fit.to(device),
                                   ...
                                   local_reg=local_reg,shrinkage=shrinkage, gene_info=gene_info.to(device),cell_sampling=args.cell_sampling,gene_sampling=args.gene_sampling)
